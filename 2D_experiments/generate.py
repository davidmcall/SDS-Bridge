...
parser.add_argument('--mode', type=str, default='bridge', choices=['bridge', 'sds', 'nfsd', 'vsd'])
...
save_dir = 'results/%s_gen/%s_lr%.3f_seed%d_scale%.1f' % (args.mode, args.prompt.replace(' ', '_'), args.lr, args.seed, args.cfg_scale)
...
for step in tqdm(range(args.n_steps)):
    ...
    if step % 100 == 0:
        imageio.mimwrite(os.path.join(save_dir, 'debug_optimization.mp4'), np.stack(im_opts).astype(np.float32)*255, fps=10, codec='libx264')