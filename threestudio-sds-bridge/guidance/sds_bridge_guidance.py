...
class SDSBridgeGuidance(BaseObject):
    @dataclass
    ...
    def compute_grad_sds_bridge(
        ...
        uncond_text_embedding = src_text_embeddings[1:]
        if self.phase_id == 1:
            text_embeddings = torch.cat([uncond_text_embedding, tgt_text_embedding])
        else:
            text_embeddings = torch.cat([src_text_embedding, tgt_text_embedding])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        grad, grad_img, guidance_eval_utils = self.compute_grad_sds_bridge(
            latents,
            rgb_BCHW_512,
            t,
            prompt_utils,
